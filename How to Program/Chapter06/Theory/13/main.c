...
// function main gebins program execution
int main(void) {
...
void mean(const int answer[]) {
    ...
    printf("The mean is the average value of the data\n"
        ...
        "all the data items divided by teh number\n"
        "of data items (%u). The mean value for\n"
        "this run is: %u / %u = %.4f\n\n",
        SIZE, total, SIZE, (double) total / SIZE);
}

// sort array adn determine median element's value
void median(int answer[]) {
    printf("\n%s\n%s\n%s\n%s", "--------", "  Median", "--------",
...
// determine most freuqent response
void mode(int freq[], const int answer[]) {
    printf("\n%s\n%s\n%s\n", "--------", " Mode", "--------");
    ...
    for (size_t rating = 1; rating <= 9; ++rating) {
        ...
        printf("\nThe mode is the ost frequent value.\n"
            "For this run the mode is %d which occurred %d time.\n",
                modeValue, largest);
}
